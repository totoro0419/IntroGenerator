...
export class AssetStore{
 ...
 async finish(){for(const a of this.items){if(a.needsRaster){if(a.width>2048||a.height>2048)throw Error('Effect素材が大きすぎます');const img=await loadImage('data:image/svg+xml;base64,'+b64(a.bytes));let c=document.createElement('canvas');c.width=Math.ceil(a.width*2);c.height=Math.ceil(a.height*2);c.getContext('2d').drawImage(img,0,0,c.width,c.height);let blob=await new Promise(r=>c.toBlob(r,'image/png'));a.bytes=new Uint8Array(await blob.arrayBuffer());a.format='png';a.hash=digest(a.bytes);a.res=2;a.cx*=2;a.cy*=2}a.name='ig_'+a.hash+'_0';a.md5=md5hex(a.bytes);a.url=`data:image/${a.format==='svg'?'svg+xml':'png'};base64,${b64(a.bytes)}`;a.image=await loadImage(a.url)}}
}
