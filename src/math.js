...
export function validateTempo(tempo){if(!Array.isArray(tempo)||!tempo.length)throw Error('Tempo Mapがありません');for(let i=0;i<tempo.length;i++){const t=tempo[i];if(!Number.isFinite(t.beat)||!Number.isFinite(t.bpm)||t.bpm<=0)throw Error('Tempo Mapの値が不正です');if(i===0&&t.beat!==0)throw Error('Tempo Mapの先頭Beatは0である必要があります');if(i>0&&t.beat<=tempo[i-1].beat)throw Error('Tempo MapのBeatは厳密に増加する必要があります')}return true}
...
export function beatAtSeconds(tempo,seconds){if(seconds<0)return seconds*tempo[0].bpm/60;let elapsed=0;for(let i=0;i<tempo.length;i++){const current=tempo[i],next=tempo[i+1];if(!next)return current.beat+(seconds-elapsed)*current.bpm/60;const span=(next.beat-current.beat)*60/current.bpm;if(seconds<=elapsed+span)return current.beat+(seconds-elapsed)*current.bpm/60;elapsed+=span}return tempo.at(-1).beat}
