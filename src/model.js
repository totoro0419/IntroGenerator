...
import {beats,random} from './math.js';
...
export function syncTiming(p){for(const a of p.timeAnchors||[]){const v=a.unit==='beats'?beats(p.tempo,a.value):a.value;if(a.target==='keyframe')p.tracks.find(t=>t.id===a.owner).keys.find(k=>k.id===a.key).time=v;else if(a.target==='audioStart')p.audio.find(x=>x.id===a.owner).start=v;else p.nodes.find(n=>n.id===a.owner).time[a.target==='nodeStart'?'start':'end']=v}for(const t of p.tracks){t.keys.sort((a,b)=>a.time-b.time);if(t.keys.some((k,i)=>i&&k.time===t.keys[i-1].time))throw Error('BPM変更でキーフレームが重なります')}for(const n of p.nodes)if(n.time.start>n.time.end)throw Error('開始が終了より後です')}
