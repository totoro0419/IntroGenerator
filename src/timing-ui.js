...
function matchingKeyTrack(rows){const p=state()?.p;if(!p)return null;const near=(a,b)=>Math.abs(a-b)<1e-6;const candidates=p.tracks.filter(tr=>tr.keys.length===rows.length&&rows.every((r,i)=>{const k=tr.keys[i],a=timingAnchor(p,'keyframe',tr.id,k.id),time=r.querySelector('input[aria-label="Key位置"]'),value=r.querySelector('input[aria-label="Key値"]'),unit=r.querySelector('select[aria-label="時間単位"]');return time&&value&&unit&&near(Number(time.value),a?.value??k.time)&&near(Number(value.value),k.value)&&unit.value===(a?.unit||'seconds')}));return candidates.length===1?candidates[0]:null}
