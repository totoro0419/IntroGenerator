...
try{
 ...
 await page.waitForFunction(()=>{const s=window.__IG?.state,n=s?.p?.nodes.find(n=>n.id===s.id);return n?.type==='repeater'&&n.data.mode==='lattice'},undefined,{timeout:60000});
 await page.waitForFunction(()=>window.__IG.state.compiledRevision===window.__IG.state.revision,undefined,{timeout:90000});
 await page.waitForSelector('[data-repeating-background-controls]',{timeout:30000});
 ...
 const change=async(label,value,check)=>{const input=page.getByLabel(label,{exact:true});await input.fill(String(value));await input.press('Tab');await page.waitForFunction(check,{id:repeaterId,value},{timeout:30000});await page.waitForFunction(()=>window.__IG.state.compiledRevision===window.__IG.state.revision&&document.querySelector('#save-state')?.textContent==='保存済み',undefined,{timeout:90000})};
 ...
 await page.waitForFunction(id=>window.__IG.state.p.nodes.find(n=>n.id===id).data.basis[1][1]===50,repeaterId,{timeout:30000});
 await page.waitForFunction(()=>window.__IG.state.compiledRevision===window.__IG.state.revision,undefined,{timeout:90000});
 await page.locator('#redo').click();
 await page.waitForFunction(id=>window.__IG.state.p.nodes.find(n=>n.id===id).data.basis[1][1]===54,repeaterId,{timeout:30000});
 await page.waitForFunction(()=>window.__IG.state.compiledRevision===window.__IG.state.revision,undefined,{timeout:90000});
 ...
 await page.waitForFunction(id=>window.__IG.state.p.nodes.find(n=>n.id===id)?.data.shape==='circle',templateId,{timeout:30000});
