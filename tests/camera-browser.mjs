...
try{
 ...
 const result=await page.evaluate(async()=>{
  ...
  const runtime=new __IG.Runtime(__IG.state.compiled),pick=(frame,id)=>frame.find(x=>x.nodeId===id);
  ...
  const scale=d=>Math.hypot(d.v[0],d.v[1]),delta=(x,y)=>[y.v[4]-x.v[4],y.v[5]-x.v[5]],angle=v=>Math.atan2(v[1],v[0])*180/Math.PI,visualWidth=d=>runtime.assets[d.asset-1].width*scale(d);
  const d0=delta(a0,b0),d1=delta(a1,b1),mid1=[(a1.v[4]+b1.v[4])/2,(a1.v[5]+b1.v[5])/2];
  ...
  return {d0,d1,mid1,angle1:angle(d1),aVisual0:visualWidth(a0),aVisual1:visualWidth(a1),bVisual0:visualWidth(b0),bVisual1:visualWidth(b1),overlay0:[o0.v[4],o0.v[5],visualWidth(o0)],overlay1:[o1.v[4],o1.v[5],visualWidth(o1)],sourceCamera:JSON.stringify({transform:cam.transform,data:cam.data}),restoredCamera:JSON.stringify({transform:restored.transform,data:restored.data})};
 });
 ...
 if(!close(result.bVisual0/result.aVisual0,2)||!close(result.bVisual1/result.aVisual1,2))throw Error(`PDF-F10 camera overwrote element-local scale: ${JSON.stringify({a0:result.aVisual0,b0:result.bVisual0,a1:result.aVisual1,b1:result.bVisual1})}`);
