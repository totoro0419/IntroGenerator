...
try{
 ...
 await page.waitForFunction(()=>__IG.state.compiledRevision===__IG.state.revision,{timeout:60000});
 const result=await page.evaluate(async()=>{
  ...
  const scale=v=>Math.hypot(v[0],v[1]),delta=(x,y)=>[y[4]-x[4],y[5]-x[5]],length=v=>Math.hypot(...v),angle=v=>Math.atan2(v[1],v[0])*180/Math.PI;
 ...
 if(errors.length)throw Error(errors.join('\n'));console.log({camera:'PDF-F10 pass',...result});
